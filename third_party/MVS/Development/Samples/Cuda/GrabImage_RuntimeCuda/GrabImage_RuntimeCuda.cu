...
static  unsigned int __stdcall WorkThread(void* pUser)
{
    ...
    unsigned char* cuDevPtr = nullptr;
    ...
    while (true)
    {
        ...
        if (nRet == MV_OK)
        {
            ...
            if (nFrameLen < stOutFrame.stFrameInfo.nFrameLenEx && nullptr != cuDevPtr)
            {
                cudaFree(cuDevPtr);
                cuDevPtr = nullptr;
            }
            ...
            if (nullptr == cuDevPtr)
            {
                cudaError = cudaMalloc(&cuDevPtr, stOutFrame.stFrameInfo.nFrameLenEx);
                if (cudaSuccess == cudaError)
                {
                    nFrameLen = stOutFrame.stFrameInfo.nFrameLenEx;
                }

                printf("cuMemAlloc status %d\n", cudaError);
            }

            if (nullptr != cuDevPtr)
            {

                cudaError = cudaMemcpy(cuDevPtr, stOutFrame.pBufAddr, stOutFrame.stFrameInfo.nFrameLenEx, cudaMemcpyHostToDevice);
            }
...
int main()
{
    ...
    do
    {
        ...
        for (unsigned int i = 0; i < BUFFER_NUMBER; i++)
        {
            ...
            cudaError = cudaHostAlloc((void**)&pBuffer[i], nPayloadSize, cudaHostAllocDefault);
            if (cudaSuccess != cudaError || nullptr == pBuffer[i])
            {
                printf("cudaHostAlloc returned %d\n-> %s\n", (int)cudaError, cudaGetErrorString(cudaError));
                break;
            }

            printf("cuMemAllocHost index %d: %p size %I64d \r\n", i, pBuffer[i], nPayloadSize);

            // ch:向SDK注册缓存 | en:Announce buffer for SDK
            nRet = MV_CC_RegisterBuffer(handle, pBuffer[i], nPayloadSize, NULL);
            if (MV_OK != nRet)
            {
