...
> **IPC_Station2（第二工位）**：第一工位坡口/Po_Kou 字段已移除；当前 payload 以 `quality_code` 为主，检测流程待实现。

- **产生时机**：
  - **正式**：PLC `Trig_Inspection` → `InspectionHandler` → `HmiTcpServer::publishInspectionResult`（当前 Handler 占位 Res=8）
  - **调试**：显控 `cmd.debug_trigger_inspection`（占位推送，不写 PLC）
- **推送策略**：`resultCode=1`（OK）与 `resultCode=2`（NG）及未实现（如 8）**均可推送**；显控须处理失败/未实现场景。
- **连接初始化**：显控 TCP 接入成功后，Core **一次性**推送 `event.inspection.finished`，`resultCode=0`，`quality_code=0`，`message="等待检测"`。
- **payload 字段（当前第二工位骨架）**：
  - `resultCode` (int): **0=尚未检测/连接占位**，1=OK, 2=NG，8=流程未实现（占位）
  - `ngReasonWord0`, `ngReasonWord1` (int)
  - `measureItemCount` (int)
  - `sourcePointCount` (int)
  - `quality_code` (int): 质量码（第二工位检测实现后由算法填充）
  - `message` (string): 概要描述
...
| `cmd.debug_trigger_inspection` | `{}` | - | **占位**：推送未实现 `event.inspection.finished`（Res=8）；不写 PLC |
