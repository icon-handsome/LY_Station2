...
CloudPtr preprocess(const CloudConstPtr& input, const MeasureConfig& cfg)
{
	...
	// ?®π?
    CloudPtr cropped = cropCloud(transformed, cfg.crop);

	// ¶À??????????
    CloudPtr filtered(new Cloud);
    if (cfg.statisticalMeanK > 0 && cropped->size() > static_cast<std::size_t>(cfg.statisticalMeanK))
	{
...
CloudPtr buildStraightSideCloud(const CloudConstPtr& scan, const MeasureConfig& cfg, const FitReport&)
{
    ...
    if (straight && !straight->empty())
    {
        ...
        if (saveRc == 0)
        {
            std::cout << "???®π?????????π◊" << straightPath << " ??????" << straight->size() << std::endl;
        }
        else
        {
            std::cout << "???®π????????????" << straightPath << " ??????" << saveRc << std::endl;
        }
    }
    else
    {
        std::cout << "???®π?????????¶ƒ????" << std::endl;
    }
    */
    return straight;
}
...
CloudPtr buildStraightSideFeatureCloud(const CloudConstPtr& scan, const MeasureConfig& cfg)
{
    ...
    if (featureCloud && !featureCloud->empty())
    {
        ...
        if (saveRc == 0)
        {
            std::cout << "???????????????π◊" << featurePath << " ??????" << featureCloud->size() << std::endl;
        }
        else
        {
...
CylinderModel fitCylinderByPcaAxis(const CloudConstPtr& cloud, const MeasureConfig& cfg)
{
    ...
    for (int iter = 0; iter < maxIter && inliers.size() >= 10; ++iter)
    {
        ...
        double robustThreshold = std::max(cfg.cylinderDistanceThresholdMm, 3.0 * 1.4826 * medianAbs);
        if (robustThreshold <= 0.0 || !std::isfinite(robustThreshold))
        {
            robustThreshold = cfg.cylinderDistanceThresholdMm > 0.0 ? cfg.cylinderDistanceThresholdMm : 2.0;
        }
    ...
    {
        radius = radiusSum / static_cast<double>(radialDistances.size());
    }
                                               ...
                                               const MeasureConfig& cfg)
{
    ...
    /* ????PCD????????????????????????????????????
        ...
        if (sliceSaveRc == 0) 
		{
            std::cout << "???????????π◊" << slicePath.str() << " ??????" << sliceCloud->size() << std::endl;
        } 
		else 
		{
                                    ...
                                    double& diameterMm)
{
    ...
    for (int y0 = 1; y0 < height - 1; ++y0)
    {
        ...
        {
            const int seed = y0 * width + x0;           // ????????¶ƒ?????????????????????????????????????????????????ß·??????????????????
            ...
            if (touchesBorder || area < 10 || boundaryCount < 4)  // ????????????ß≥????????????????????
            {
                continue;
            }
    ...
    // 2D??????????????????????????????????????????
    // ????????øÇ????3D?????????ó®???2D??????projection_direction???????????
                                    ...
                                    int& windowCount)
{
    ...
    const double baseMad = baseAbsResiduals.empty() ? 0.0 : baseAbsResiduals[baseAbsResiduals.size() / 2]; // ????????ß÷????ß”????¶À?????????
    const double residualThreshold = std::max(cfg.straightBResidualMinThresholdMm, 1.5 * 1.4826 * baseMad); // 1.4826*MAD??????????sigma??1.5?????????ßÿ????
                                        ...
                                        double& heightMm)
{
	...
	std::vector<double> straight_edge_tapers;           // ???ß“??
    ...
    */
	...
	deviationMm = medianValue(straight_edge_tapers);   // ???ß“????????
...
MeasureResult MeasurePipeline::runPipelineWithPreprocessedScan(const CloudConstPtr& scan)
{
	...
	*scanInTemplate = *scan;
    ...
    /* ????PCD????????????????????????????????????
		...
		if (scanInTemplate && !scanInTemplate->empty())
		{
			...
			if (saveRc == 0)
			{
				std::cout << "????????????π◊" << alignedPath << " ??????" << scanInTemplate->size() << std::endl;
			}
			else
			{
    ...
    */
    ...
    std::cout << "straight_side_points=" << straightSide->size() << '\n';
