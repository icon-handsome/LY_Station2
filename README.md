...
| 节 | 键 | 说明 |
|----|-----|------|
| `[FlowControl]` | `scanCacheDirectory` | 分段采集缓存根目录；空则 `<exe>/ScanTracking_CaptureCache` |
| `[FlowControl]` | `retainSegmentPly` | `true` 时检测后保留磁盘 PLY/PGM（默认） |
...
落盘目录结构、数据流与联调说明见 [`docs/算法使用API.md`](docs/算法使用API.md) §10.1 与 [`docs/多点位扫描与位姿跟踪完整流程.md`](docs/多点位扫描与位姿跟踪完整流程.md)（v1.3 §「分段采集磁盘缓存」）。

## 开发文档

| 主题 | 文档 |
|------|------|
| 未完成项 / 进度 | [`docs/项目未完成事项清单_v1.0.md`](docs/项目未完成事项清单_v1.0.md)（v1.5） |
...
| Modbus 协议 | [`docs/封头检测工位PLC-IPC Modbus通信协议_v0.2.md`](docs/封头检测工位PLC-IPC%20Modbus通信协议_v0.2.md) |
...
- `LB` / `LBN` / 蓝友第一工位检测已接入主流程。
- **分段扫描**：`Trig_ScanSegment` 成功后点云与海康图落盘至 `ScanTracking_CaptureCache`；`Trig_Inspection` 从磁盘加载外/内/孔三段 PLY 解算（非全部段融合）。
- 目前程序已可在本机启动。
