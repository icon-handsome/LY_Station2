...
- `resultCode`：0=占位，1=OK，2=NG，3=缓存/数据异常
- `ngReasonWord0` / `ngReasonWord1`
- `measureItemCount`、`sourcePointCount`
- `quality_code`：1=缓存校验通过（占位），2=段数据无效
- `message`

数据链路（Core 内）：

```text
ScanSegmentCache → evaluateStation2Inspection
...
void onJsonEnvelope(const QJsonObject& envelope) {
    ...
    const int qualityCode = payload.value(QStringLiteral("quality_code")).toInt();
    const QString message = payload.value(QStringLiteral("message")).toString();
    // resultCode==2/3 时也要更新 UI
}
```
