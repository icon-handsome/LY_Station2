...
| 4 | 海康 2D 任一台已连接 | 海康 A/B 均未连接（服务已配置时） |
| 5 | Tracking 模块已加载 | （保留） |
...
- **产生时机**：算法处理完成（`Trig_Inspection` → `TrackingService::inspectSegments`）。
- **Core 实现状态**：自 v1.0 起，下列测量字段均由 `InspectionMeasurement` 结构化写入 payload（非仅 `message` 文本）；检测失败时数值字段为 `0`。
- **payload 字段**（算法参数全集）：
