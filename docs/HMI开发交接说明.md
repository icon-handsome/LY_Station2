...
- 调试路径：`cmd.debug_trigger_inspection` 用状态机**内存点云缓存**跑检测，**不写 PLC、不清缓存**；须 `allowDebugTriggerInspection=true`。
- **多路径**：缓存可按 `segmentIndex` 存多段，蓝友仍只取 `[Tracking]` 三个段位（外/内/孔）；路径级融合见未完成清单 §2.3.2。
