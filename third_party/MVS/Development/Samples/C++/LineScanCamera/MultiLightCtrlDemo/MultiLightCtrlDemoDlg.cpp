...
int CReconstructImageDemoDlg::GrabThreadProcess()
{
    ...
    while(m_bThreadState)
    {
        ...
        if (nRet == MV_OK)
        {
            if (0 < stImageInfo.stFrameInfo.nSubImageNum && EXPOSURE_NUM >= stImageInfo.stFrameInfo.nSubImageNum)
            {
                // 卡端拆图不支持HB，不用考虑HB解码
                for (unsigned int i = 0; i < stImageInfo.stFrameInfo.nSubImageNum; i++)
                {
                    stImageData.nWidth = stImageInfo.stFrameInfo.SubImageList.pstSubImage[i].nWidth;
                    stImageData.nHeight = stImageInfo.stFrameInfo.SubImageList.pstSubImage[i].nHeight;
                    stImageData.enPixelType = stImageInfo.stFrameInfo.SubImageList.pstSubImage[i].enPixelType;
                    stImageData.nImageLen = stImageInfo.stFrameInfo.SubImageList.pstSubImage[i].nImageLen;
                    stImageData.pImageBuf = stImageInfo.stFrameInfo.SubImageList.pstSubImage[i].pImageBuf;
            ...
            else
            {
                ...
                if (m_nMultiLightNum > 1 && m_nMultiLightNum <= EXPOSURE_NUM)      // 多灯
                {
                    ...
                    for (unsigned int i = 0; i < m_nMultiLightNum; i++)
                    {
                        ...
                        stImageData.nImageLen = stImgReconstructionParam.stDstBufList[i].nBufLen;
                ...
                else
                {
                    if (bHBFalg)
                    {
                        ...
                        stImageData.nImageLen = stHBParam.nDstBufLen;
