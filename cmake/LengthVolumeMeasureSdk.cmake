...
set(
    SCAN_TRACKING_LENGTH_VOLUME_MEASURE_SDK_DIR
    "${CMAKE_CURRENT_SOURCE_DIR}/third_party/length_volume_measure"
    CACHE PATH
    "Path to the LengthVolumeMeasure SDK directory (headers/lib/bin)"
)

set(_SCAN_TRACKING_LENGTH_VOLUME_PCL_DLLS
    pcl_common.dll
    pcl_filters.dll
    pcl_kdtree.dll
    pcl_search.dll
...
function(scan_tracking_require_length_volume_measure_sdk)
    if(TARGET LengthVolumeMeasureSdk::Api)
        return()
    endif()

    set(_sdk_dir "${SCAN_TRACKING_LENGTH_VOLUME_MEASURE_SDK_DIR}")
    set(_include_dir "${_sdk_dir}/include")
    set(_release_lib "${_sdk_dir}/lib/Release/LengthVolumeMeasure.lib")
    set(_debug_lib "${_sdk_dir}/lib/Debug/LengthVolumeMeasure.lib")
    set(_release_dll "${_sdk_dir}/bin/Release/LengthVolumeMeasure.dll")
    set(_debug_dll "${_sdk_dir}/bin/Debug/LengthVolumeMeasure.dll")
    set(_header "${_include_dir}/length_volume_measure_c_api.h")
    ...
    foreach(_required_path IN ITEMS
        ...
        if(NOT EXISTS "${_required_path}")
            message(FATAL_ERROR
                "LengthVolumeMeasure SDK file not found: ${_required_path}\n"
                "Build LengthVolumeMeasureDll and run scripts/package_sdk_to_ipc.ps1 in the algorithm repo.")
        endif()
    endforeach()
    ...
    add_library(LengthVolumeMeasureSdk::Api SHARED IMPORTED GLOBAL)
    set_target_properties(LengthVolumeMeasureSdk::Api PROPERTIES
    ...
    set_property(GLOBAL PROPERTY SCAN_TRACKING_LENGTH_VOLUME_MEASURE_SDK_DIR "${_sdk_dir}")
    set_property(GLOBAL PROPERTY SCAN_TRACKING_LENGTH_VOLUME_MEASURE_DLL_RELEASE "${_release_dll}")
    set_property(GLOBAL PROPERTY SCAN_TRACKING_LENGTH_VOLUME_MEASURE_DLL_DEBUG "${_debug_dll}")
endfunction()

function(scan_tracking_deploy_length_volume_measure_runtime target_name)
    scan_tracking_require_length_volume_measure_sdk()
    ...
    foreach(_pcl_dll IN LISTS _SCAN_TRACKING_LENGTH_VOLUME_PCL_DLLS)
    ...
    if(EXISTS "${_config_ini}")
        list(APPEND _copy_cmds
            COMMAND ${CMAKE_COMMAND} -E make_directory
                "$<TARGET_FILE_DIR:${target_name}>/config/length_volume_measure"
            COMMAND ${CMAKE_COMMAND} -E copy_if_different
                "${_config_ini}"
    ...
    add_custom_command(TARGET ${target_name} POST_BUILD
        ${_copy_cmds}
        COMMENT "Deploying LengthVolumeMeasure SDK runtime"
    )
