    ...
    int radius)
{
    ...
    if (p00 && p10 && p01 && p11 && isFinitePoint(*p00) && isFinitePoint(*p10) &&
        isFinitePoint(*p01) && isFinitePoint(*p11)) {
    ...
    for (int yy = clampInt(y0 - radius, 0, height - 1); yy <= clampInt(y0 + radius, 0, height - 1);
        ...
        for (int xx = clampInt(x0 - radius, 0, width - 1); xx <= clampInt(x0 + radius, 0, width - 1);
             ++xx) {
            const cv::Point3f* point = cloudAt(cloud, yy, xx);
            if (point == nullptr || !isFinitePoint(*point)) {
                continue;
            }
            const double dist2 = (xx - x) * (xx - x) + (yy - y) * (yy - y);
            const double weight = 1.0 / std::max(dist2, 1e-6);
            sumW += weight;
            sx += weight * point->x;
            sy += weight * point->y;
            sz += weight * point->z;
...
Result Estimator::estimate(const cv::Mat& grayImage, const AlignedOrganizedCloud& alignedCloud)
{
    ...
    if (!detectMarkerCenters2d(grayImage, result.centers2d)) {
